...
class General(unittest.TestCase):
    ...
    def test_general(self):
        ...
        for i in range(4):
            Input.General_Input.simulate('Credit')
...
class Test_Common_Input(Test):
    ...
    def test_Test(self):
        ...
        Input.General_Input.simulate('Test')
        ...
        self.assertEqual(Common.TEST_MODE, True)
        Input.General_Input.simulate('Test')
        Input.General_Input.simulate('Tilt')
        self.assertEqual(Common.TEST_MODE, False)
        
        Input.General_Input.simulate('Test')
        self.assertEqual(Common.TEST_MODE, True)
        Input.General_Input.simulate('Test')
        Input.General_Input.simulate('Slam')
        self.assertEqual(Common.TEST_MODE, False)
...
class Test_Playfield(Test):
    ...
    def test_Point(self):
        input = Input.General_Input.simulate('Point')
        self.assertEqual(Common.get_all_scores()[0], input.points)
        
    def test_Point_Light(self):
        input = Input.General_Input.simulate('Point_Light')
        pts = input.points[0]
        self.assertEqual(Common.get_all_scores()[0],  pts)
        input.lamp.set_level(1)
        Input.General_Input.simulate('Point_Light')
        pts += input.points[1]
        self.assertEqual(Common.get_all_scores()[0], pts)
        
    def test_Point_Light_Blink(self):
        input = Input.General_Input.instances['Point_Light_Blink']
        ...
        while off == False or on == False:
            ...
            if level == 0 and off == False: 
                Input.General_Input.simulate('Point_Light_Blink')
                off = True
    ...
    def test_Spinner(self):
        input = Input.General_Input.instances['Spinner']
        Input.General_Input.simulate('Spinner')
        input.lamp.set_level(1)
        Input.General_Input.simulate('Spinner')
        self.assertEqual(input.lamp.get_level(), 0)
          
    def test_Target_Bank(self):
        target_0 = Input.General_Input.instances['0_Target_Bank']
        target_1 = Input.General_Input.instances['1_Target_Bank']
        
        self.assertEqual(target_0.lamp.get_level(), "blink")
        self.assertEqual(target_1.lamp.get_level(), "blink")
        
        input = Input.General_Input.simulate(target_0.name)
        pts = input.points[0]
        self.assertEqual(target_0.lamp.get_level(), 1)
        
        input = Input.General_Input.simulate(target_1.name)
        pts += input.points[0]
    ...
    def test_Drop_Target_Bank(self):
        target_0 = Input.General_Input.instances['0_Drop_Target_Bank']
        target_1 = Input.General_Input.instances['1_Drop_Target_Bank']
        
        Input.General_Input.simulate(target_1.name)
        self.assertEqual(target_1.level, 1)
        Input.General_Input.simulate(target_0.name) # Reset execute apres la chute de la derniere cible
        self.assertEqual(target_0.level, 0)
        
    def test_Target_Bank_2_states(self):
        target_0 = Input.General_Input.instances['0_Target_Bank_2_states']
        target_1 = Input.General_Input.instances['1_Target_Bank_2_states']
